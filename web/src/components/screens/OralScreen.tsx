    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <div className="relative px-5 lg:px-8 py-4 flex items-center gap-3 border-b border-slate-800/80">
        ...
        <div className="flex items-center gap-2">
          <Logo size={32} />
          <span className="font-extrabold text-[15px]">{t.oral.simTitle}</span>
        </div>
        <div className="ms-auto flex items-center gap-4">
          <div className="flex items-center gap-2 text-[12px]">
            <span className="w-2 h-2 bg-rose-500 rounded-full animate-pulse" />
            <span className="font-bold text-rose-300 uppercase tracking-wider">{t.oral.rec}</span>
      ...
      <div className="relative grid lg:grid-cols-12 gap-6 px-5 lg:px-8 py-6">
        ...
        <div className="lg:col-span-6 flex flex-col items-center">
          ...
          <div className="text-center mb-3">
            {stage === 'idle' && (
              <>
                <div className="text-[13px] uppercase tracking-wider text-sky-400 font-bold mb-2">{t.oral.readyToBegin}</div>
                ...
                <button onClick={() => setStage('running')}
                  ...
                  <span>{t.oral.startOral}</span>
                  <span className="ltr">▶</span>
                </button>
              </>
            )}
            {stage === 'running' && (
              <div className="text-[12px] uppercase tracking-wider text-sky-400 font-bold">{t.oral.listening}</div>
            )}
            {stage === 'finished' && (
              <div className="text-[12px] uppercase tracking-wider text-emerald-400 font-bold">{t.oral.examFinished}</div>
            )}
          </div>
          ...
          <div className="mt-6 flex items-center gap-2">
            ...
            {stage === 'finished' && (
              <Btn kind="primary" size="lg" onClick={() => { bumpStreak(120); router.push(`/${locale}/progress`); }}>
                {t.oral.seeReport} <span className="ltr">→</span>
              </Btn>
            )}
          </div>
        </div>
...
function ExaminerCard() {
  ...
  return (
    <div className="bg-slate-900/70 backdrop-blur border border-slate-800 rounded-2xl p-4">
      <div className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500 mb-3">{t.oral.examiner}</div>
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="font-bold text-slate-100">{t.oral.examinerName}</div>
          <div className="text-[11.5px] text-slate-400">{t.oral.examinerRole}</div>
        </div>
      </div>
      <div className="mt-3 text-[11.5px] text-slate-400 bg-slate-800/60 rounded-lg p-2 leading-relaxed">
        {t.oral.examinerQuote}
      </div>
    </div>
  );
}
...
function SessionInfoCard() {
  const { t } = useApp();
  const rows = [
    { k: t.oral.topic,      v: t.oral.topicVal },
    { k: t.oral.difficulty, v: t.oral.difficultyVal },
    { k: t.oral.accent,     v: t.oral.accentVal },
    { k: t.oral.length,     v: t.oral.lengthVal }
  ];
  return (
    <div className="bg-slate-900/70 backdrop-blur border border-slate-800 rounded-2xl p-4">
      <div className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500 mb-3">{t.oral.session}</div>
          ...
          <div key={i} className={`flex ${isStudent ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5
              ...
              <div className={`text-[10.5px] font-bold uppercase tracking-wider mb-1 ${isStudent ? 'text-sky-300' : 'text-violet-300'}`}>
                {isStudent ? t.oral.roleStudent : t.oral.examiner}
              </div>
...
function RubricPanel({ scores, stage }: { scores: Scores; stage: Stage }) {
  const { t } = useApp();
  const items = [
    { key: 'pronunciation' as const, label: t.oral.pronunciation, icon: '🗣️' },
    { key: 'confidence'    as const, label: t.oral.confidence,    icon: '💪' },
    { key: 'accuracy'      as const, label: t.oral.accuracy,      icon: '🎯' },
    { key: 'structure'     as const, label: t.oral.structure,     icon: '🏗️' }
  ];
  ...
  return (
    <div className="bg-slate-900/70 backdrop-blur border border-slate-800 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500">{t.oral.liveRubric}</div>
        <div className="text-[10.5px] text-violet-300 font-bold ltr">assessment.agent</div>
      </div>

      <div className="mb-5 text-center">
        <div className="text-[11px] text-slate-400 uppercase tracking-wider font-bold mb-1">{t.oral.overall}</div>
          ...
          return (
            <div key={it.key}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[12.5px] font-semibold text-slate-200">{it.label}</span>
